...
run_seir_model <- function(j, beta, gamma, sigma, prop_s,  
  ...
  travel_mat[is.na(travel_mat)] <- 0                     # make sure NAs are 0
  N_eff <- c(pop_vec %*% travel_mat)                     # set effective population
  repeated_N_eff <- replicate(length(N_eff), N_eff)      # repeat eff. pop. for computation
  repeated_N <- replicate(length(pop_vec), pop_vec)      # set population
  mobility_mat_0 <- travel_mat                           # make a travel matrix w/ 0 in diagonal
  diag(mobility_mat_0) <- 0
  
  # Loop through time steps
  for (i in 2:n_steps) {
    
    # FOI Equation 2 from https://www.medrxiv.org/content/medrxiv/early/2024/03/11/2023.11.22.23298916.full.pdf

    # Create necessary variables
    I_eff <- c(i_mat[, i-1] %*% travel_mat)                           # set effective infected pop.
    repeated_I <- t(replicate(length(i_mat[, i-1]), i_mat[, i-1]))    # repeat infected pop. for computation
    repeated_I_eff <- t(replicate(length(I_eff), I_eff))              # repeat eff. infected pop. for computation
    ...
    # Calculate the probability of exposure
