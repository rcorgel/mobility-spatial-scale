################################################################################
# File Name: 04a_metapop_results_intro_loc_sens                                #
#                                                                              #
...
adm_3_at_2_mp_sens_1 <- NULL
adm_3_at_1_mp_sens_1 <- NULL
adm_3_at_2_mp_sens_2 <- NULL
adm_3_at_1_mp_sens_2 <- NULL
...
for (i in seq(1, 2, 1)) {
  for (j in seq(1, 330, 1)) {
    ...
    adm_3 <- run_seir_model_multi(n = 50, density_dep = FALSE, method = 'append',
                                 R_0 = 1.5, gamma = 1/7, sigma = 1/2, prop_s = 0.90, 
                                 adm_name_vec = adm_3_name_vec, adm_level = '3', 
                                 pop_vec = adm_3_pop_vec, intro_adm = 'All', intro_num = j,
                                 adm_x_walk = adm_3_x_walk, travel_mat = as.matrix(mobility_dat_adm_3[[i]]), 
                                 max_time = 365, time_step = 1, mobility = TRUE)
    ...
    adm_3_at_2 <- adm_3 %>% group_by(time, adm_2, run_num) %>%
      ...
      mutate(intro_time = mean(time),
             intro_loc = j) %>%
      slice(1) %>% dplyr::select(intro_loc, magnitude, intro_time)
    ...
    if (i == 1) {
      adm_3_at_2_mp_sens_1 <- rbind(adm_3_at_2_mp_sens_1, adm_3_at_2)
      adm_3_at_1_mp_sens_1 <- rbind(adm_3_at_1_mp_sens_1, adm_3_at_1)
    }
    if (i == 2) {
      adm_3_at_2_mp_sens_2 <- rbind(adm_3_at_2_mp_sens_2, adm_3_at_2)
      adm_3_at_1_mp_sens_2 <- rbind(adm_3_at_1_mp_sens_2, adm_3_at_1)
    }
  }
}
...
for (i in seq(1, 2, 1)) {
  for (j in seq(1, 25, 1)) {
    ...
    adm_2 <- run_seir_model_multi(n = 50, density_dep = FALSE, method = 'append',
                                 ...
                                 adm_x_walk = adm_2_x_walk, travel_mat = as.matrix(mobility_dat_adm_2[[i]]), 
                                 max_time = 365, time_step = 1, mobility = TRUE)
