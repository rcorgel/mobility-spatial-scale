################################################################################
# File Name: 04b_metapop_results_trans_sens                                    #
#                                                                              #
# Purpose:   Examine differences in metapopulation model results between       #
#            different mobility spatial scale across transmissibility values.  #
# Steps:                                                                       # 
#            1. Set-up script                                                  #
...
adm_3_at_2_mp_sens_1_col <- NULL
adm_3_at_1_mp_sens_1_col <- NULL
adm_3_at_2_mp_sens_2_col <- NULL
adm_3_at_1_mp_sens_2_col <- NULL
adm_3_at_2_mp_sens_1_mad <- NULL
adm_3_at_1_mp_sens_1_mad <- NULL
adm_3_at_2_mp_sens_2_mad <- NULL
adm_3_at_1_mp_sens_2_mad <- NULL

# Create object for mobility data
mobility_dat_adm_3 <- list(adm_3_phone_mobility_mat_rescale_adm_1, adm_3_phone_mobility_mat_rescale_adm_2)

# n = 50; density_dep = FALSE; method = 'append';
# R_0 = 1.5; gamma = 1/7; sigma = 1/2; prop_s = 0.90; 
# adm_name_vec = adm_3_name_vec; adm_level = '3';
# pop_vec = adm_3_pop_vec; intro_adm = 'All'; intro_num = 1;
  ...
  for (k in seq(1, 2, 1)) {
    for (j in seq(1, 4, 0.1)) {
      ...
      adm_3 <- run_seir_model_multi(n = 50, density_dep = FALSE, method = 'append',
                                    R_0 = j, gamma = 1/7, sigma = 1/2, prop_s = 0.90, 
                                    adm_name_vec = adm_3_name_vec, adm_level = '3', 
                                    pop_vec = adm_3_pop_vec, intro_adm = i, intro_num = 1,
                                    adm_x_walk = adm_3_x_walk, travel_mat = as.matrix(mobility_dat_adm_3[[k]]), 
                                    max_time = 365, time_step = 1, mobility = TRUE)
      ...
      if (i == 'Colombo') {
        if (k == 1) {
          adm_3_at_2_mp_sens_1_col <- rbind(adm_3_at_2_mp_sens_1_col, adm_3_at_2)
          adm_3_at_1_mp_sens_1_col <- rbind(adm_3_at_1_mp_sens_1_col, adm_3_at_1)
        }
        if (k == 2) {
          adm_3_at_2_mp_sens_2_col <- rbind(adm_3_at_2_mp_sens_2_col, adm_3_at_2)
          adm_3_at_1_mp_sens_2_col <- rbind(adm_3_at_1_mp_sens_2_col, adm_3_at_1)
        }
      }
      if (i == 'Madhu') {
        if (k == 1) {
          adm_3_at_2_mp_sens_1_mad <- rbind(adm_3_at_2_mp_sens_1_mad, adm_3_at_2)
          adm_3_at_1_mp_sens_1_mad <- rbind(adm_3_at_1_mp_sens_1_mad, adm_3_at_1)
        }
        if (k == 2) {
          adm_3_at_2_mp_sens_2_mad <- rbind(adm_3_at_2_mp_sens_2_mad, adm_3_at_2)
          adm_3_at_1_mp_sens_2_mad <- rbind(adm_3_at_1_mp_sens_2_mad, adm_3_at_1)
        }
      }
    }
  }
...
# Create empty objects to fill
adm_2_at_2_mp_sens_1_col <- NULL
adm_2_at_1_mp_sens_1_col <- NULL
adm_2_at_2_mp_sens_2_col <- NULL
adm_2_at_1_mp_sens_2_col <- NULL
adm_2_at_2_mp_sens_1_mad <- NULL
adm_2_at_1_mp_sens_1_mad <- NULL
adm_2_at_2_mp_sens_2_mad <- NULL
adm_2_at_1_mp_sens_2_mad <- NULL
  ...
  for (k in seq(1, 2, 1)) {
    for (j in seq(1, 4, 0.1)) {
      adm_2 <- run_seir_model_multi(n = 50, density_dep = FALSE, method = 'append',
                                    ...
                                    adm_x_walk = adm_2_x_walk, travel_mat = as.matrix(mobility_dat_adm_2[[k]]), 
                                    max_time = 365, time_step = 1, mobility = TRUE)
      ...
      if (i == 'Colombo') {
        if (k == 1) {
          adm_2_at_2_mp_sens_1_col <- rbind(adm_2_at_2_mp_sens_1_col, adm_2_at_2)
          adm_2_at_1_mp_sens_1_col <- rbind(adm_2_at_1_mp_sens_1_col, adm_2_at_1)
        }
        if (k == 2) {
          adm_2_at_2_mp_sens_2_col <- rbind(adm_2_at_2_mp_sens_2_col, adm_2_at_2)
          adm_2_at_1_mp_sens_2_col <- rbind(adm_2_at_1_mp_sens_2_col, adm_2_at_1)
        }
      }
      if (i == 'Madhu') {
        if (k == 1) {
          adm_2_at_2_mp_sens_1_mad <- rbind(adm_2_at_2_mp_sens_1_mad, adm_2_at_2)
          adm_2_at_1_mp_sens_1_mad <- rbind(adm_2_at_1_mp_sens_1_mad, adm_2_at_1)
        }
        if (k == 2) {
          adm_2_at_2_mp_sens_2_mad <- rbind(adm_2_at_2_mp_sens_2_mad, adm_2_at_2)
          adm_2_at_1_mp_sens_2_mad <- rbind(adm_2_at_1_mp_sens_2_mad, adm_2_at_1)
        }
      }
    }
  }
...
# Create object for mobility data
mobility_dat_adm_1 <- list(adm_1_phone_mobility_mat)
  ...
  for (k in seq(1, 1, 1)) {
    for (j in seq(1, 4, 0.1)) {
      adm_1 <- run_seir_model_multi(n = 50, density_dep = FALSE, method = 'append',
                                    ...
                                    adm_x_walk = adm_1_x_walk, travel_mat = as.matrix(mobility_dat_adm_1[[k]]), 
                                    max_time = 365, time_step = 1, mobility = TRUE)
...
# Save results
save(list = c('adm_3_at_1_mp_sens_1_col',
              'adm_3_at_2_mp_sens_2_col', 
              'adm_2_at_1_mp_sens_1_col', 
              'adm_2_at_2_mp_sens_2_col', 
              'adm_1_at_1_mp_col',
              'adm_3_at_1_mp_sens_1_mad',
              'adm_3_at_2_mp_sens_2_mad', 
              'adm_2_at_1_mp_sens_1_mad', 
              'adm_2_at_2_mp_sens_2_mad', 
              'adm_1_at_1_mp_mad'), 
     file = './tmp/transmission_model_results_obs_sens.RData')
