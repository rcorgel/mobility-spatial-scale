...
source('./mobility-spatial-scale/04_metapop_model_3.R')
...
for (i in seq(1, 330, 1)) {
  ...
  # Run 100 iterations
  adm_3 <- mclapply(1, run_seir_model, beta = 0.2142857, gamma = 1/7, sigma = 1/2, prop_s = 0.90,
...
saveRDS(adm_3_at_2_mp, 'tmp/adm_3_at_2_mp_no_stoc_re')
saveRDS(adm_3_at_1_mp, 'tmp/adm_3_at_1_mp_no_stoc_re')
...
for (i in seq(1, 25, 1)) {
  ...
  # Run 100 iterations
  adm_2 <- mclapply(1, run_seir_model, beta = 0.2142857, gamma = 1/7, sigma = 1/2, prop_s = 0.90,
                    adm_name_vec = adm_2_name_vec, adm_level = '2',
                    pop_vec = adm_2_pop_vec, intro_adm = 'All', intro_num = i,
  ...
  # Convert to ADM 2
  adm_2_at_2 <- adm_2_results  |> group_by(time, adm_2, run_num) |>
    # Calculate incidence at Admin 2
    mutate(incid_I_adm_2 = sum(incid_I)) |>
    distinct(time, run_num, adm_2, incid_I_adm_2) |> 
    ungroup() |>
  ...
  # Convert to ADM 1
  adm_2_at_1 <- adm_2_results |> group_by(time, adm_1, run_num) |>
    # Calculate incidence at Admin 1
    mutate(incid_I_adm_1 = sum(incid_I)) |>
    distinct(time, run_num, adm_1, incid_I_adm_1) |> 
    ungroup() |>
...
saveRDS(adm_2_at_2_mp, 'tmp/adm_2_at_2_mp_no_stoc_re')
saveRDS(adm_2_at_1_mp, 'tmp/adm_2_at_1_mp_no_stoc_re')
...
for (i in seq(1, 9, 1)) {
  ...
  # Run 100 iterations
  adm_1 <- mclapply(1, run_seir_model, beta = 0.2142857, gamma = 1/7, sigma = 1/2, prop_s = 0.90,
  ...
  # Append results together
  adm_1_results <- do.call(rbind, adm_1)
  
  # Drop results list
  remove(adm_1)
  
  # Convert to ADM 1
  adm_1_at_1 <- adm_1_results |> group_by(time, adm_1, run_num) |>
    # Calculate incidence at Admin 1
    mutate(incid_I_adm_1 = sum(incid_I)) |>
    distinct(time, run_num, adm_1, incid_I_adm_1) |> 
    ungroup() |>
...
saveRDS(adm_1_at_1_mp, 'tmp/adm_1_at_1_mp_no_stoc_re')
...
# Detect the number of available cores (optional, mclapply does this by default)
num_cores <- detectCores()

library(pbmcapply)
start.time <- Sys.time()
adm_3_mp <- pbmclapply(1:3, run_seir_model_multi_adm_3, data = adm_3_phone_mobility_mat,
                     mc.cores = num_cores)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken
...
run_seir_model_multi_adm_3 <- function(intro_num, data) {
  ...
  adm_2 <- run_seir_model_multi(n = 100, density_dep = FALSE, method = 'append',
                                R_0 = 1.1, gamma = 1/7, sigma = 1/2, prop_s = 0.90, 
                                adm_name_vec = adm_2_name_vec, adm_level = '3', 
                                pop_vec = adm_3_pop_vec, intro_adm = 'All', intro_num = intro_num,
                                adm_x_walk = adm_3_x_walk, travel_mat = data, 
                                max_time = 365, time_step = 1, mobility = TRUE, mixing = FALSE,
                                mixing_mat = 0)
